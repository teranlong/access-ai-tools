...
```bash
python -c "import sys; sys.path.insert(0,'.agents/skills/access-migration-tools'); from catalog import VERSION; print(VERSION)"
```
...
```bash
python .agents/skills/access-migration-tools/catalog.py "<project_directory_path>" --charts
```
